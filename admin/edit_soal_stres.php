...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id_soal'])) {
    ...
    if (!empty($id_soal) && !empty($questionText) && !empty($kode) && isset($nilaiA) && isset($nilaiB) && isset($nilaiC) && isset($nilaiD)) {
        ...
        $query = "UPDATE questions_s SET kode = ?, question_text = ?, nilai_a = ?, nilai_b = ?, nilai_c = ?, nilai_d = ? WHERE id_soal = ?";
        $stmt = mysqli_prepare($koneksi, $query);
        mysqli_stmt_bind_param($stmt, "ssiiiii", $kode, $questionText, $nilaiA, $nilaiB, $nilaiC, $nilaiD, $id_soal);

        if (mysqli_stmt_execute($stmt)) {
            header("Location: soal_stres.php?success=update");
            exit();
        } else {
...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT * FROM questions_s WHERE id_soal = ?";
    $stmt = mysqli_prepare($koneksi, $query);
    ...
    header("Location: soal_stres.php");
    exit();
}
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h2 class="card" style="background-color: #69BE9D; color: white; padding: 25px 50px;">Edit Gejala Stres</h2>
                    <a href="soal_stres.php" class="btn btn-primary mt-3 mb-4"><i class="fa-solid fa-angle-left mr-2"></i> Kembali </a>
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form method="post" action="edit_soal_stres.php">
